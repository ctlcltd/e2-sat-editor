...
- [ ] preferences, freeze on settings save during FTP transfer, `tab::ftpComboItems`, no debug trace [gui] [settings.cpp] [tab.cpp]
- [ ] cli history, no stepping, history loads, `std::iostream` `EOF` [cli] [e2db_termctl.cpp] [windows]
- [ ] reference panel, `QWidget` background transparency, scrollbar decorations [qt] [kde] [linux]
- [x] preferences, segfault on settings save, `qt-6.4.2`, `tab::ftpComboChanged` `QComboBox::setCurrentIndex` [gui] [settings.cpp] [tab.cpp]
- [x] find match cache, is cached after drop, drag and drop [gui] [mainView.cpp] [viewAbstract.cpp]
...
- [x] mainView side, `QTreeWidget` background transparency, scrollbar decorations [qt] [linux]
...
- [x] preferences, corner menus and `QSettings` synchronization [gui] [settings.cpp]
...
- [x] find match cache, is cached after channel deletion [gui] [viewAbstract.cpp]
...
- [ ] status bar closed, visual feedback `tab::infoMessage` downloaded files, no double dialog errors and infos [gui] [tab.cpp]
...
- [ ] FTPS, HTTPS support [networking] [fptcom] [gui] [settings.cpp]
- [ ] lamedb 2.5, comments support [e2db]
- [ ] neutrino webtv support (issues:#35) [e2db] [gui]
...
- [ ] accessibility status [a11y] [gui]
- [ ] HiDPI, font sizes [gui]
- [ ] intl. support, `LRM` text is not enough [rtl] [gui]
...
- [ ] singular userbouquet import, in original sorting order, `e2db_parser::parse_e2db_bouquet` [e2db]
...
- [ ] import from ftp, tools and demo, expending `e2db::import_blob` `e2db_parser::parse_e2db` [e2db]
